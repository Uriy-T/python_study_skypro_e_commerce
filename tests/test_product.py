...
from data_for_tests.product_test_data.test_datasets import \
    (product_create_by_classmethod_invalid_data
     as cm_invalid_data)
from data_for_tests.product_test_data.test_datasets import (
    set_price_with_incorrect_type as incorrect_price)

from fixtures.entities_fixtures import create_product_valid
...
# Позитивные тесты создания продукта
    ...
    @pytest.mark.product_positive
    def test_create_product_by_classmethod(self,
                                           create_product_by_classmethod: Product
                                           ) -> None:
        assert (
...
### Негативные тесты создания через метод класса
    ...
    @pytest.mark.product_negative
    def test_create_product_by_classmethod_without_without_description(self) -> None:
        with pytest.raises(KeyError) as exc_info:
    ...
    @pytest.mark.product_negative
    def test_create_product_by_classmethod_without_without_quantity(self) -> None:
        with pytest.raises(KeyError) as exc_info:
...
# Позитивные тесты логики изменения параметра price
    @pytest.mark.product_positive
    def test_set_price_more_than_actual(self,
    ...
    def test_set_price_less_than_actual_apply(self,
                                              mock_input,
                                              create_product_valid):
        assert (
                create_product_valid.price
                == valid_data[0]["expected_values"]["price"]
        )
    ...
    def test_set_price_less_than_actual_decline(self,
                                                mock_input,
                                                create_product_valid):
        assert (
                create_product_valid.price
        ...
        create_product_valid.price = 10000
        assert (
                create_product_valid.price == 57000)
        mock_input.assert_called_once()
...
# Установка отрицательного и нулевого значения параметра price
    ...
    def test_set_zero_price(self,
        ...
        assert captured_message.out == 'Цена не должна быть нулевая или отрицательная\n'
...
# Установка невалидных типов для значения параметра price
    ...
    @pytest.mark.product_negative
    @pytest.mark.parametrize(param_packer_old(incorrect_price),
                             value_packer_old(incorrect_price))
    def test_set_price_with_incorrect_data_type(self,
                                                create_product_valid: Product,
                                                new_price: Any
                                                ) -> None:
        with pytest.raises(TypeError) as exc_info:
