...
class TestProduct:
    ...
    def test_create_product_positive(self,
                                     create_product_valid: Product) -> None:
        assert (create_product_valid.name ==
                valid_data[0]['expected_values']['name'])
        assert (create_product_valid.description ==
                valid_data[0]['expected_values']['description'])
        assert (create_product_valid.price ==
                valid_data[0]['expected_values']['price'])
        assert (create_product_valid.quantity ==
                valid_data[0]['expected_values']['quantity'])

    @pytest.mark.product_negative
    def test_create_product_without_name(self) -> None:
        with pytest.raises(TypeError) as exc_info:
            Product(description=invalid_data[0]['data_for_create']['description'],
    ...
    def test_create_product_without_description(self) -> None:
        with pytest.raises(TypeError) as exc_info:
            Product(name=invalid_data[1]['data_for_create']['name'],
    ...
    def test_create_product_without_price(self) -> None:
        with pytest.raises(TypeError) as exc_info:
            Product(name=invalid_data[2]['data_for_create']['name'],
    ...
    def test_create_product_without_quantity(self) -> None:
        with pytest.raises(TypeError) as exc_info:
            Product(name=invalid_data[3]['data_for_create']['name'],
    ...
    @pytest.mark.product_negative
    @pytest.mark.parametrize(param_packer_old(data_type), value_packer_old(data_type))
    def test_create_product_with_incorrect_types(self,
