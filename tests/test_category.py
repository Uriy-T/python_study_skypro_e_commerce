...
# Позитивные тесты добавления продукта
    ...
    def test_add_new_product(self):
        ...
        assert (category.get_products[0] ==
                'Xbox series X, 56000.0 руб.'
                f' Остаток: 30')

        category.add_product(product2)
        assert len(category.get_products) == 2
        assert (category.get_products[1] ==
                'NES "reborn", 4500.5 руб.'
                f' Остаток: 11')

    @pytest.mark.category_positive
    def test_add_exist_product_more_price(self):
        product1 = Product(
            "Xbox series X", "Консоль + 2 геймпада", 56000.00, 30
        )
        product2 = Product(
        ...
        assert len(category.get_products) == 2
        assert (category.get_products[0] ==
                'Xbox series X, 100000.0 руб.'
                f' Остаток: 42')

    @pytest.mark.category_positive
    def test_add_exist_product_less_price(self):
        product1 = Product(
            "Xbox series X", "Консоль + 2 геймпада", 56000.00, 30
        )
        product2 = Product(
...
# Негативные тесты добавления продукта
    @pytest.mark.category_negative
    @pytest.mark.parametrize(param_packer_old(incorrect_product),
                             value_packer_old(incorrect_product))
    def test_add_product_incorrect_type(self,
                                        create_category: Category,
                                        add_product: Any) -> None:
        with pytest.raises(TypeError) as exc_info:
            create_category.add_product(add_product)

            assert str(exc_info.value) == 'add_product должен быть типом Product'
